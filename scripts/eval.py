...
SCANREFER_TRAIN = json.load(open(os.path.join(CONF.PATH.DATA, "train_attr.json")))
SCANREFER_VAL = json.load(open(os.path.join(CONF.PATH.DATA, "val_attr.json")))
...
def get_model(args, config):
    ...
    model_name = "model_last.pth" if args.last else "model.pth"
...
def eval_det(args):
    ...
    if gen_flag:
        ...
        for seed in seeds:
            ...
            sem_acc_all.append(np.mean(sem_acc))
            for i, ap_calculator in enumerate(AP_CALCULATOR_LIST):
                print()
                print("-"*10, "iou_thresh: %f"%(AP_IOU_THRESHOLDS[i]), "-"*10)
                metrics_dict = ap_calculator.compute_metrics()
                if AP_IOU_THRESHOLDS[i] == 0.25:
                    mAP25_all.append(metrics_dict['mAP'])
                if AP_IOU_THRESHOLDS[i] == 0.5:
                    mAP50_all.append(metrics_dict['mAP'])
                for key in metrics_dict:
                    print("eval %s: %f"%(key, metrics_dict[key]))
        print('sem acc avg:', np.mean(sem_acc_all))
        print('mAP25 avg:', np.mean(mAP25_all))
        print('mAP50 avg:', np.mean(mAP50_all))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prepare_epoch", type=float, help="prepare epoch for pretask.", default=0)
    ...
    parser.add_argument("--last", action="store_true", help="fully supervised.")
