...
def get_eval(data_dict, config, reference, use_lang_classifier=False, use_oracle=False, use_cat_rand=False, use_best=False, post_processing=None,
             relation_prediction=False, color_prediction=False, shape_prediction=False, size_prediction=False, prepare=False):
    """ Loss functions
    ...
    # lang
    if (reference or pretask) and use_lang_classifier:
    ...
    # pretask
    if relation_prediction and not prepare:
        _total = data_dict['rel_correct'] + data_dict['rel_wrong']
        data_dict['relation_acc'] = _total if _total == 0 else data_dict['rel_correct'].true_divide(_total)
    else:
        data_dict['relation_acc'] = torch.zeros(1)[0].cuda()
    
    if color_prediction and not prepare:
        _total = data_dict['col_correct'] + data_dict['col_wrong']
        data_dict['color_acc'] = _total if _total == 0 else data_dict['col_correct'].true_divide(_total)
    else:
        data_dict['color_acc'] = torch.zeros(1)[0].cuda()

    if shape_prediction and not prepare:
        _total = data_dict['shape_correct'] + data_dict['shape_wrong']
        data_dict['shape_acc'] = _total if _total == 0 else data_dict['shape_correct'].true_divide(_total)
    else:
        data_dict['shape_acc'] = torch.zeros(1)[0].cuda()

    if size_prediction and not prepare:
        _total = data_dict['size_correct'] + data_dict['size_wrong']
        data_dict['size_acc'] = _total if _total == 0 else data_dict['size_correct'].true_divide(_total)
    else:
        data_dict['size_acc'] = torch.zeros(1)[0].cuda()
    ...
    if data_dict["pred_mask"].sum() == 0:
        data_dict["sem_acc"] = data_dict["pred_mask"].sum()
    else:
