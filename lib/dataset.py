...
class ScannetReferenceDataset(Dataset):
    ...
    def __init__(self, scanrefer, scanrefer_all_scene, 
        ...
        # sample bbox
        if sampled_box != 10000 and split == 'train':
            self.sampled_box = sampled_box
        else:
            self.sampled_box = 10000
    ...
    def __getitem__(self, idx):
        ...
        if self.color_prediction or self.shape_prediction or self.size_prediction:
            attribute_raw = self.scanrefer[idx]["object_attrbution_id"] # dic

        # get relations in language
        relmat = np.zeros((MAX_NUM_REL, 5)) - 100 # ignore -100
        num_rel = np.zeros(1)
        if self.relation_prediction:
            relation_raw = self.scanrefer[idx]["relation"] # list
            for i, rel in enumerate(relation_raw):
                if i >= MAX_NUM_REL:
                    break
                if rel["subject"]["sub_lemma_head"] == rel["object"]["obj_lemma_head"]:
                    continue
                sub_name = self.sync(rel["subject"]["sub_lemma_head"])
                sub_cat = self.raw2label[sub_name] if sub_name in self.raw2label else 17
                obj_name = self.sync(rel["object"]["obj_lemma_head"])
                obj_cat = self.raw2label[obj_name] if obj_name in self.raw2label else 17
                # print("sub_cat", sub_name, sub_cat, "obj_cat", obj_name, obj_cat)

                sub_inst = -1 if (rel["subject"]["sub_lemma_head"]) not in object_name.replace('_', ' ') else object_id
                obj_inst = -1 if (rel["object"]["obj_lemma_head"]) not in object_name.replace('_', ' ') else object_id

                relmat[i] = np.ascontiguousarray(np.array([sub_cat, obj_cat, rel["relation_id"], sub_inst, obj_inst], dtype=np.int64))
                num_rel += 1
            # support for distribution
            if num_rel == 0:
                relmat[0] = np.ascontiguousarray(np.array([17, 17, 8, -1, -1], dtype=np.int64))
                num_rel += 1
        
        # get color attribution in language
        colmat = np.zeros((1, 2)) - 100 # ignore -100 max:1
        num_col = np.zeros(1)
        if self.color_prediction:
            tmp_name = self.raw2label[object_name] if object_name in self.raw2label else 17
            for i, col_id in enumerate(attribute_raw["color"]):
                if i >= 1:
                    break
                colmat[i] = np.ascontiguousarray(np.array([int(tmp_name), int(col_id)], dtype=np.int64))
                num_col += 1
            # support for distribution
            if num_col == 0:
                colmat[0] = np.ascontiguousarray(np.array([int(tmp_name), int(11)], dtype=np.int64))
                num_col += 1

        # get shape attribution in language
        shapemat = np.zeros((1, 2)) - 100 # ignore -100 max:1
        num_shape = np.zeros(1)
        if self.shape_prediction:
            tmp_name = self.raw2label[object_name] if object_name in self.raw2label else 17
            for i, shape_id in enumerate(attribute_raw["shape"]):
                if i >= 1:
                    break
                shapemat[i] = np.ascontiguousarray(np.array([int(tmp_name), int(shape_id)], dtype=np.int64))
                num_shape += 1
            # support for distribution
            if num_shape == 0:
                shapemat[0] = np.ascontiguousarray(np.array([int(tmp_name), int(7)], dtype=np.int64))
                num_shape += 1

        # get size attribution in language
        sizemat = np.zeros((1, 2)) - 100 # ignore -100 max:1
        num_size = np.zeros(1)
        if self.size_prediction:
            tmp_name = self.raw2label[object_name] if object_name in self.raw2label else 17
            for i, size_id in enumerate(attribute_raw["size"]):
                if i >= 1:
                    break
                # print("size_id", size_id)
                sizemat[i] = np.ascontiguousarray(np.array([int(tmp_name), int(size_id)], dtype=np.int64))
                num_size += 1
            # support for distribution
            if num_size == 0:
                sizemat[0] = np.ascontiguousarray(np.array([int(tmp_name), int(8)], dtype=np.int64))
                num_size += 1
        ...
        if self.split != "test":
            ...
            if self.sampled_box != 10000:
                num_bbox = min(instance_bboxes.shape[0], self.sampled_box)
                if num_bbox < instance_bboxes.shape[0]:
                    sampled_box = []
                    for i in range(num_bbox):
                        sampled_box.append(i)
                    instance_bboxes = instance_bboxes[sampled_box]
            ...
            size_gts[0:num_bbox, :] = target_bboxes[0:instance_bboxes.shape[0], 3:6]
        ...
        data_dict["relmat"] = relmat.astype(np.int64) # (MAX_NUM_REL, 5)
        data_dict["colmat"] = colmat.astype(np.int64) # (1, 2)
        data_dict["shapemat"] = shapemat.astype(np.int64) # (1, 2)
        data_dict["sizemat"] = sizemat.astype(np.int64) # (1, 2)
        data_dict["num_rel"] = num_rel.astype(np.int64)
        data_dict["num_col"] = num_col.astype(np.int64)
        data_dict["num_shape"] = num_shape.astype(np.int64)
        data_dict["num_size"] = num_size.astype(np.int64)
